...
users=$(http --print=b --follow
http://gold.experts.library.ucdavis.edu:8081/experts/query query=='PREFIX ucdrp:
<http://experts.ucdavis.edu/schema#> select ?userId WHERE { [] a ucdrp:person;
...
for id in $users; do dc exec harvest harvest -v add_user --search=userId=$id --init --remove; done
