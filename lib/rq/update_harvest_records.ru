...
WHERE {
	GRAPH harvest_oap: {
    { select ?publication ?exemplarRecord ?a_native ?publication_number ?experts_id WHERE {
      { select ?publication ?exemplarRecord (min(?native) as ?a_native) WHERE {
        ...
        bind(uri(concat(str(experts_pub:),?pub_id)) as ?experts_id)
		    {
      ...
      bind(uri(concat(str(experts_pub:),?publication_number)) as ?experts_id)
    }}
  }
}
